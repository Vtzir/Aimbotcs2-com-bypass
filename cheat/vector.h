...
static int screenWidthh = GetSystemMetrics(SM_CXSCREEN);
static int screenHeightt = GetSystemMetrics(SM_CYSCREEN);
...
struct Vector3
{
	...
	Vector3 WTS(view_matrix_t matrix) const
	{
		...
		float x = screenWidthh / 2;
		float y = screenHeightt / 2;

		x += 0.5f * _x * screenWidthh + 0.5f;
		y -= 0.5f * _y * screenHeightt + 0.5f;
